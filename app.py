# ==============================================================================
# STREAMLIT VERSION - FULLY FIXED: Documentation Display + PNG Fallback + Baselines
# File: app.py
# ==============================================================================
...
import base64
import json  # For JSON export fallback

# ==============================================================================
# 📚 ACADEMIC DOCUMENTATION MODULE
# ==============================================================================
...
def render_academic_documentation():
    ...
    with st.expander("📚 Academic Documentation: Methodology, Parameters & Scenario Analysis", expanded=True):
        ...
        **Government Tools Under Status Quo**:
        ...
        - Goodwin, P., Dargay, J., & Hanly, M. (2004). Elasticities of road traffic and fuel consumption. *Transport Reviews, 24*(3), 275–292.
...
class OilMarketStackelberg:
    ...
    def follower_response(self, cost_per_liter, scenario, subsidy_amount, price_cap):
        ...
        if scenario == "Status Quo (Deregulation)":
            ...
            final_price = min(price_with_tax, price_cap) if price_cap > 0 else price_with_tax
...
def generate_scenario_chart(fx_rate, subsidy_billions):
    ...
    fig.update_layout(yaxis_title="Price (PHP per Liter)", template="plotly_white", height=400, margin=dict(t=30,b=30,l=30,r=30), showlegend=False)
    return fig
...
def generate_projection_chart(start_price, fx_rate, scenario, subsidy, cap, n_simulations=30, seed=42):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dates.tolist()+dates.tolist()[::-1], y=upper_bound.tolist()+lower_bound.tolist()[::-1],
        fill='toself', fillcolor='rgba(75,149,255,0.2)', line=dict(color='rgba(255,255,255,0)'), hoverinfo='skip', name='90% CI'))
    fig.add_trace(go.Scatter(x=dates, y=mean_prices, mode='lines', line=dict(color='#2E86AB',width=3), name='Projected Price (Mean)'))
    fig.add_trace(go.Scatter(x=[dates[0]], y=[start_price], mode='markers', marker=dict(color='#E63946',size=10,symbol='star'), name='Starting Price'))
    fig.update_layout(title="📈 Price Projection: March 22 - April 22, 2026", xaxis_title="Date", yaxis_title="Price (PHP per Liter)",
        template="plotly_white", height=400, margin=dict(t=50,b=30,l=30,r=30), hovermode='x unified', xaxis_tickformat="%b %d")
...
# ✅ Render academic documentation (NOW WORKS - no orphaned code)
...
# ==============================================================================
# ⚙️ SIDEBAR CONTROLS (BASELINES SET TO $98/bbl, ₱60/USD)
# ==============================================================================
with st.sidebar:
    ...
    if use_live:
        api_key = st.text_input("Alpha Vantage API Key", type="password", help="Get free key: https://www.alphavantage.co/support/#api-key", placeholder="Enter your API key...")
    ...
    scenario = st.selectbox("Policy Scenario", ["Status Quo (Deregulation)", "Repeal Deregulation (Price Cap)", "Stockpile Strategy"])
    
    # ✅ FIXED: Scenario explanations now appear AFTER scenario is defined
...
with col1: st.metric("📉 Landed Cost", f"₱{cost:.2f}/L")
with col2: st.metric("🏷️ Pump Price", f"₱{price:.2f}/L", delta=f"{price-cost:.2f} margin")
with col3: st.metric("⚠️ Supply Risk", risk)
...
with st.spinner("🔄 Generating projection..."):
    fig_line, proj_dates, proj_prices = generate_projection_chart(price, fx_rate, scenario, subsidy_billions, price_cap, n_simulations=30, seed=42)
...
with col_p1: st.metric("📅 End Price", f"₱{proj_prices[-1]:.2f}/L")
with col_p2: st.metric("📊 31-Day Change", f"{((proj_prices[-1]-price)/price)*100:+.1f}%")
with col_p3: st.metric("📉 Volatility", f"{np.std(np.diff(proj_prices))/np.mean(proj_prices)*100:.1f}%")
...
with col_d1:
    df_scenarios = pd.DataFrame({
        'Scenario': ["1. $200/bbl + Status Quo", "2. <$200/bbl + Status Quo", "3. $200/bbl + Repeal", "4. <$200/bbl + Stockpile"],
        'Pump_Price_PHP': [round(model.run_simulation(200,fx_rate,0,0,"Status Quo (Deregulation)")[0],2),
                          round(model.run_simulation(82,fx_rate,0,0,"Status Quo (Deregulation)")[0],2),
                          round(model.run_simulation(200,fx_rate,subsidy_billions,90,"Repeal Deregulation (Price Cap)")[0],2),
                          round(model.run_simulation(82,fx_rate,subsidy_billions,0,"Stockpile Strategy")[0],2)],
        'Crude_Price_USD': [200,82,200,82], 'Policy': ['Status Quo','Status Quo','Repeal','Stockpile'], 'FX_Rate_PHP': [fx_rate]*4
    })
...
with col_d2:
    df_projection = pd.DataFrame({'Date': proj_dates, 'Projected_Price_PHP': [round(p,2) for p in proj_prices], 'Scenario': scenario, 'Crude_Price_Input': crude_price})
...
st.markdown("---")
st.caption("""
**Research Disclaimer:** Prototype model for academic purposes. Actual prices may vary based on market conditions, geopolitical events, and policy implementation.
